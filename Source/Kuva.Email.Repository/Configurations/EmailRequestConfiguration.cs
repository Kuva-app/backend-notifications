...
public sealed class EmailRequestConfiguration : IEntityTypeConfiguration<EmailRequest>
{
    public void Configure(EntityTypeBuilder<EmailRequest> builder)
    {
        ...
        builder.HasIndex(x => x.TemplateCode).HasDatabaseName("IX_EmailRequests_TemplateCode");
        builder.HasIndex(x => x.Status).HasDatabaseName("IX_EmailRequests_Status");
        builder.HasIndex(x => x.ExternalReference).HasDatabaseName("IX_EmailRequests_ExternalReference");
        builder.HasIndex(x => x.CreatedAtUtc).HasDatabaseName("IX_EmailRequests_CreatedAtUtc");
        builder.HasIndex(x => new { x.TemplateCode, x.ExternalReference }).HasDatabaseName("IX_EmailRequests_TemplateCode_ExternalReference");
    }
}
