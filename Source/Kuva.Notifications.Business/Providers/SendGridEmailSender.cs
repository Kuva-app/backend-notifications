...
public sealed class SendGridEmailSender(HttpClient httpClient, IConfiguration configuration) : INotificationSender
{
    public NotificationType Type => NotificationType.Email;
    public NotificationProviderType ProviderType => NotificationProviderType.SendGrid;
    ...
    public async Task<NotificationSendResult> SendAsync(RenderedNotification notification, CancellationToken cancellationToken)
    {
        ...
        {
            return NotificationSendResult.Fail("SendGridNotConfigured", "SendGrid provider is not configured.");
        }
        ...
        request.Content = JsonContent.Create(new SendGridMessage(
            ...
            [new SendGridPersonalization(notification.Recipients.Where(x => x.Role.Equals("To", StringComparison.OrdinalIgnoreCase)).Select(x => new SendGridAddress(x.Address, x.Name)).ToArray())],
            notification.Subject,
            [new SendGridContent("text/html", notification.HtmlBody)]));

        var response = await httpClient.SendAsync(request, cancellationToken);
        return response.IsSuccessStatusCode
            ? NotificationSendResult.Ok(response.Headers.TryGetValues("X-Message-Id", out var values) ? values.FirstOrDefault() : null)
            : NotificationSendResult.Fail(response.StatusCode.ToString(), "SendGrid provider failed to send the email.");
    }
